...
Make shure that you enabled connection with prometheus in `$HOME/.rebusd/config/config.toml`, if not:
```
...
```
Add variables `denom` and `bench_prefix` for Rebus
```
denom=arebus
bench_prefix=rebus
...
CPUWeight=95
IOWeight=95
ExecStart=cosmos-exporter --denom $denom --denom-coefficient 1000000000000000000 --bech-prefix $bench_prefix
Restart=always
RestartSec=2
LimitNOFILE=800000
KillSignal=SIGTERM
...
```
curl -sSL https://raw.githubusercontent.com/AlexToTheSun/Validator_Activity/main/Testnet-guides/Rebus/Monitoring/prometheus.yml > $HOME/prometheus/prometheus.yml
```
...
```
cd $HOME/prometheus
wget -q -O add_node.sh https://raw.githubusercontent.com/AlexToTheSun/Validator_Activity/main/Testnet-guides/Rebus/Monitoring/add_node.sh && chmod +x add_node.sh
```
