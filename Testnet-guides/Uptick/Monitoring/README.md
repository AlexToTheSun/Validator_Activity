## Monitoring with a centralized type of log storage for Uptick
...
Make shure that you enabled connection with prometheus in `$HOME/.uptickd/config/config.toml`, if not:
```
...
```
Add variables `denom` and `bench_prefix` for Uptick
```
denom=auptick
bench_prefix=uptick
...
```
curl -sSL https://raw.githubusercontent.com/AlexToTheSun/Validator_Activity/main/Testnet-guides/Uptick/Monitoring/prometheus.yml > $HOME/prometheus/prometheus.yml
```
