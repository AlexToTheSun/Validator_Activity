## Monitoring with a centralized type of log storage for Umee
...
```
Add variables `denom` and `bench_prefix` for Umee
```
denom=uumee
bench_prefix=umee
...
```
curl -sSL https://raw.githubusercontent.com/AlexToTheSun/Validator_Activity/main/Mainnet-Guides/Umee/Monitoring/prometheus.yml > $HOME/prometheus/prometheus.yml
```
...
```
cd $HOME/prometheus
wget -q -O add_node.sh https://raw.githubusercontent.com/AlexToTheSun/Validator_Activity/main/Mainnet-Guides/Umee/Monitoring/add_node.sh && chmod +x add_node.sh
```
