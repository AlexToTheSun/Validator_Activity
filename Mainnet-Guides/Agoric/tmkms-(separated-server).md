...
This server does not require the Agoric SDK. **tmkms** only.  
The official documentation is [[here](https://github.com/iqlusioninc/tmkms#tendermint-kms-)]
### Install tmkms
Update & upgrade
```
...
### Init tmkms
```
tmkms init $HOME/.tmkms/agoric
```
### Copy `priv_validator_key.json` from validator node
Use [WinSCP](https://winscp.net/download/WinSCP-5.19.5-Setup.exe) to copy the `priv_validator_key.json` from the validator's node. And put it in the `$HOME/.tmkms/agoric` folder.
Then type the command below to import information from `priv_validator_key.json` to `cosmoshub-3-consensus.key`:
```
tmkms softsign import $HOME/.tmkms/agoric/priv_validator_key.json $HOME/.tmkms/agoric/secrets/cosmoshub-3-consensus.key
rm $HOME/.tmkms/agoric/priv_validator_key.json
```
### Edit tmkms.toml
Open `tmkms.toml` in `nano`
```
nano $HOME/.tmkms/agoric/tmkms.toml
```
...
state_file = "$HOME/.tmkms/agoric/state/cosmoshub-3-consensus.json"
...
chain_ids = ["agoric-3"]
key_type = "consensus"
path = "$HOME/.tmkms/agoric/secrets/cosmoshub-3-consensus.key"
...
secret_key = "$HOME/.tmkms/agoric/home/secrets/kms-identity.key"
protocol_version = "v0.34"
reconnect = true
```
- `addr` - the validator' server address. Shoud match the `priv_validator_laddr` in agoric' `config.toml`.
- `protocol_version` - find out `ag0 tendermint version`
## Restert both validator and tmkms
### Start tmkms
Try start on tmkms server
```
tmkms start -c $HOME/.tmkms/agoric/tmkms.toml
```
If everything is alright then start tmkms as service file.  
**Create the file**
```
sudo tee <<EOF >/dev/null /etc/systemd/system/tmkmsagoric.service

[Unit]  
Description=tmkms agoric service  
...
RestartSec=10  
User=root
ExecStart=/usr/bin/tmkms start -c $HOME/.tmkms/agoric/tmkms.toml  
LimitNOFILE=1024
...
Start tmkmsagoric
```
sudo systemctl enable tmkmsagoric
sudo systemctl daemon-reload
sudo systemctl restart tmkmsagoric
journalctl -u tmkmsagoric -f
```
### Restart agoric service on the server of the validator
```
...
```
journalctl -u tmkmsagoric -f --output cat
